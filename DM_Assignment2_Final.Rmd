...
cv2_grid = foreach(k=nfolds, .combine='rbind') %do% {
lm1 = lm(children ~ hotel + adults + meal + market_segment + reserved_room_type + booking_changes + customer_type + average_daily_rate + required_car_parking_spaces + total_of_special_requests + arrival_date, data = hotels_val_folds$test, check.rows = T)

} %>% as.data.frame

nfolds = 20
hotels_val_folds = crossv_kfold(hotels_val, k=K_folds)
models2 = map(hotel_val_folds$train, lm(children ~ hotel + adults + meal + market_segment + reserved_room_type + booking_changes + customer_type + average_daily_rate + required_car_parking_spaces + total_of_special_requests + arrival_date, data = ., use.all=FALSE))
predval = map2_dbl(models2,hotels_val_folds$test, )
...
hotels_val_folds <- hotels_val %>%
  mutate(fold_id = sample(fold_id, replace=FALSE))%>%
  group_by(fold_id)%>%
  summarize(prop_children = mean(children), count_children = sum(children))

foreach(group = fold_id, .combine='rbind') %do%{
  phat_val_test = predict(lm0 , hotels_val$fold_id, type='response')
